...
export default function Home() {
  ...
  const [showGraphDeep,setShowGraphDeep] = useState(false);
  ...
  const selectSector = (id:string) => { setActiveId(id); setStage(0); setShowGraphDeep(false); document.getElementById("chapter")?.scrollIntoView({behavior:"smooth",block:"start"}); };
  ...
  return <main>
    ...
    <section id="supplements" className="supplements section darkSection">
      ...
      <article className="supplementSheet">
        ...
        <div className="supplementEquation"><span>CORE EQUATION</span><Latex>{activeSupplement.equation}</Latex></div>
    ...
    <section id="chapter" className={`chapter ${active.accent}`}>
      ...
      {active.id==='graphcast'&&<button className={`deepDiveToggle ${showGraphDeep?'open':''}`} aria-expanded={showGraphDeep} onClick={()=>setShowGraphDeep(value=>!value)}><span><small>OPTIONAL TECHNICAL APPENDIX</small><b>{showGraphDeep?'Hide the ten-part GraphCast deep dive':'Open the ten-part GraphCast deep dive'}</b></span><i>{showGraphDeep?'−':'+'}</i></button>}
      {active.id==='graphcast'&&showGraphDeep&&<div className="graphcastDeep">
