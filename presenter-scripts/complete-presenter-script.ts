/**
 * PRIVATE PRESENTER MATERIAL — intentionally outside the audience application.
 * This file is not imported by any file under app/. Keep cue text here so it is
 * available to presenters in the repository without entering the public bundle.
 */

type PresentationStageId = "objective" | "bottleneck" | "architecture" | "vocabulary" | "weights" | "lab" | "result";
type ScriptCue = { stage: PresentationStageId; text: string; cue?: "CLICK" | "POINT" | "PAUSE" | "ASK" | "ADJUST" | "OBSERVE" | "TRANSITION"; relatedNodeId?: string; relatedVocabularyTerm?: string; relatedLabControl?: string; technicalNote?: string };
type CaseScript = { title: string; quick: ScriptCue[]; full: Record<PresentationStageId, ScriptCue[]>; deeperNotes: string[] };
type SupplementScript = { quick: string[]; full: string[]; equationNote: string; failureMode: string; flagshipLinks: string[] };
